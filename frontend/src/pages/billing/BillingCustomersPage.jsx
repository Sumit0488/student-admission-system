...
export default function BillingCustomersPage() {
  ...
  const someChecked = customers.some((c) => selected.includes(c._id)) && !allChecked;
  const toggleAll = () => setSelected(allChecked ? [] : customers.map((c) => c._id));
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await getBillingCustomers(params);
  ...
  const handleCreate = async (payload) => {
    try {
      const { data } = await createBillingCustomer(payload);
      setCustomers((p) => [data.data, ...p]);
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-50 dark:border-slate-700/50 flex-wrap">
            ...
            <span className="text-xs text-gray-500 dark:text-slate-400 whitespace-nowrap">
              from <span className="font-semibold text-gray-700 dark:text-slate-300">{total.toLocaleString()}</span> Customers
            </span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
                {loading ? (
                  Array.from({ length: 4 }).map((_, i) => <tr key={i}><td className="px-4 py-3" />{COLS.map((__, j) => <td key={j} className="px-4 py-3"><div className="h-3 bg-gray-200 dark:bg-slate-700 rounded animate-pulse w-20" /></td>)}</tr>)
                ) : customers.length === 0 ? (
                  <tr><td colSpan={COLS.length + 1} className="px-6 py-16 text-center">
                    ...
                    <p className="text-sm text-gray-400 dark:text-slate-500">No customers found</p>
                  </td></tr>
                ) : (
                  customers.map((c) => (
                    <tr key={c._id} className={`hover:bg-gray-50/60 dark:hover:bg-slate-700/30 transition-colors ${selected.includes(c._id) ? 'bg-blue-50 dark:bg-blue-900/10' : ''}`}>
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <input type="checkbox" checked={selected.includes(c._id)} onChange={() => toggleOne(c._id)}
                          className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                      </td>
                      <td className="px-4 py-3 font-mono text-xs text-blue-600 dark:text-blue-400">{c.customer_id}</td>
                      <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{c.name}</td>
                      <td className="px-4 py-3 text-gray-600 dark:text-slate-300">{c.mobile_number || '—'}</td>
                      <td className="px-4 py-3 text-gray-600 dark:text-slate-300 text-xs">{c.email || '—'}</td>
                      <td className="px-4 py-3 text-gray-600 dark:text-slate-300">{c.stream || '—'}</td>

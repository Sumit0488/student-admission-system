...
import {
  GraduationCap, Search, Plus, X, Clock, CheckCircle, XCircle,
  Download, FileText, AlertCircle, ChevronRight,
} from 'lucide-react';
import { submitCertificateRequest, getStudentRequests, downloadCertificate, getTemplates } from '../../services/admissionsApi';
...
export default function CertificateRequestPage() {
  ...
  const handleLookup = async (e) => {
    ...
    if (!usn)  { setLookupError('Please enter your USN'); return; }
    if (!name) { setLookupError('Please enter your name'); return; }
    setLookupError('');
    setLookupLoading(true);
    try {
      const { data } = await getStudentRequests(usn);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">{activeName} · {activeUsn}</p>
              </div>
              <button onClick={() => { setShowModal(false); setForm(EMPTY_FORM); setFormErrors({}); }}
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 overflow-y-auto">
              <div>
                ...
                <select value={form.templateId} onChange={set('templateId')} className={inp(formErrors.templateId)}>
                  <option value="">— Select Certificate —</option>
                  {certTypes.map((t) => <option key={t._id} value={t._id}>{t.name}</option>)}
                </select>
                {formErrors.templateId && <p className="text-xs text-red-500 mt-1">{formErrors.templateId}</p>}
              </div>
      ...
      <header className="bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-sm font-bold text-gray-900 leading-none">Certificate Request Portal</h1>
          </div>
        </div>
      </header>
        ...
        {!lookupDone ? (
          <div className="max-w-md mx-auto">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-gray-500 mt-1">Enter your USN and name to view or submit certificate requests</p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
              <form onSubmit={handleLookup} className="space-y-4">
                <div>
                  ...
                  <input
                    value={usnInput}
                    onChange={(e) => { setUsnInput(e.target.value); setLookupError(''); }}
                ...
                <button type="submit" disabled={lookupLoading}
                  ...
                  {lookupLoading ? 'Loading…' : <><Search size={15} /> View My Requests</>}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  ...
                  <button onClick={() => { setLookupDone(false); setUsnInput(''); setNameInput(''); }}
            ...
            {requests.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">Your certificate requests will appear here</p>
                <button onClick={() => setShowModal(true)}
                ...
                {requests.map((r) => (
                  <div key={r._id}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-0.5">
                            Requested on {new Date(r.requestedDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}
                            {' · '}
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold ${STATUS_STYLE[r.status]}`}>
                      ...
                      {r.status === 'Approved' && r.certificateRef && r.deliveryType === 'Download' && (
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl px-5 py-4 flex items-start gap-3">
              ...
              <p className="text-xs text-blue-700">
                Certificate requests are typically processed within <strong>2–3 working days</strong>.
                For urgent requests, please contact the administration office directly.
              </p>
            </div>
          </div>
        )}

...
import { BarChart2, Download, FileText, TrendingUp, Calendar, Users, BookOpen } from 'lucide-react';
...
const REPORTS = [
  { key: 'day_book', label: 'Day Book Report', icon: Calendar, desc: 'Date-wise collection register' },
  { key: 'member_list', label: 'Member List Report', icon: Users, desc: 'All library members with status' },
  { key: 'fine_report', label: 'Fine Collection Report', icon: TrendingUp, desc: 'Library fine collection details' },
  { key: 'circulation', label: 'Circulation Report', icon: BookOpen, desc: 'Books issued and returned' },
];
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [txnRes, memRes] = await Promise.all([
        getFeeTransactions({ module_name: 'Library', limit: 1000, start: startDate, end: endDate }),
        getLibraryMembers({ limit: 1000 }),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <BarChart2 size={22} className="text-emerald-600" /> Library Reports
          </h1>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Library fee collection reports and analytics</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Members', value: members.length, color: 'bg-blue-600' },
          { label: 'Active Members', value: members.filter(m => m.status === 'Active').length, color: 'bg-green-600' },
          { label: 'Total Collected', value: fmtAmt(totalCollected), color: 'bg-emerald-600' },
          { label: 'Total Refunded', value: fmtAmt(totalRefunded), color: 'bg-orange-500' },
        ].map(c => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-slate-700">
            ...
            <button onClick={() => downloadCSV(Object.entries(byCategory).map(([cat, amt]) => ({ Category: cat, Amount: amt })), 'library-category-report')}
          ...
          <div className="p-4 space-y-3">
            {loading ? (
              Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-8 bg-gray-100 dark:bg-slate-700 rounded animate-pulse" />)
                ...
                <div className="flex-1">
                  ...
                  <div className="w-full h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full">
                    <div className="h-1.5 rounded-full bg-emerald-500"
                      style={{ width: `${Math.min(100, (amt / totalCollected) * 100)}%` }} />
                  </div>
                </div>
              </div>
            ))}

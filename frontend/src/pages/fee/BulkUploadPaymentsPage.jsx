...
import { bulkUploadPayments } from '../../services/feeApi';
...
const PAYMENT_METHODS = ['DD', 'PO', 'CHEQUE', 'CASH', 'RTGS / NEFT', 'BOOK ADJUSTMENT'];

const REQUIRED_COLUMNS = ['usn', 'pay_amount', 'method'];
...
function Step1({ onNext }) {
  ...
  const handleExport = () => {
    const header = 'usn,student_name,order_id,pay_amount,method,date,ref_no,description\n';
    const blob = new Blob([header], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'student_payments_template.csv';
    a.click();
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-gray-600 dark:text-slate-300">{PAYMENT_NOTES[0]}</p>
        <div>
          <p className="text-sm font-semibold text-gray-700 dark:text-slate-200">
            Payment Methods can be added in file with same as below
          </p>
          <ul className="mt-2 space-y-1">
            {PAYMENT_METHODS.map((m, i) => (
              <li key={m} className="text-sm text-gray-600 dark:text-slate-300">{i + 1}. {m}</li>
            ))}
          </ul>
          <p className="mt-3 text-sm font-semibold text-gray-700 dark:text-slate-200">Date Format : DD-MM-YYYY</p>
        </div>
      </div>
    </div>
  );
...
function Step2({ parsed, onNext, onBack }) {
  ...
  const handleUpload = async () => {
    ...
    setError('');
    try {
      const { data } = await bulkUploadPayments(parsed.rows);
      onNext(data.data);
    } catch (err) {
    ...
    <div className="flex flex-col items-center justify-center py-16 gap-6 text-center">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400 mt-2">
          Successfully created <strong>{result?.created}</strong> payment records.
      ...
      <div className="flex gap-3">
        <button
          onClick={() => navigate('/admin/fee/transactions')}
        ...
        <button
          ...
          className="px-6 py-2.5 text-sm font-semibold border border-gray-200 dark:border-slate-600 text-gray-700 dark:text-slate-300 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
        >
          Fee Tracker
        </button>
      </div>
...
export default function BulkUploadPaymentsPage() {
  const navigate = useNavigate();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Bulk Upload - Student Payments</h1>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-8">
        ...
        {step === 0 && (
          <Step1 onNext={(p) => { setParsed(p); setStep(1); }} />
        )}
        {step === 1 && (
          <Step2
            parsed={parsed}

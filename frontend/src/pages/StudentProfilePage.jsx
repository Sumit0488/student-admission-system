...
function Toast({ msg, type, onDismiss }) {
  ...
  return (
    <div
      className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-xl text-white text-sm font-medium
...
export default function StudentProfilePage() {
  ...
  const [certReqForm, setCertReqForm] = useState({
    templateId: '',
    reason: '',
  ...
  const handleDownloadCert = async (cert) => {
    ...
    setDlId(cert._id);
    try {
      await downloadCertificate(cert._id);
    } catch (err) {
  ...
  const handleRegenerate = async (cert) => {
    setDlId(cert._id);
    try {
      const { data } = await approveCertificate(cert._id);
      const updated = data.data;
      setStudentCerts((p) => p.map((c) => (c._id === cert._id ? { ...c, ...updated } : c)));
      showToast('PDF regenerated successfully!');
    } catch (err) {
      let msg = err?.response?.data?.error || err.message || 'Regeneration failed';
      if (msg.includes('Template is incomplete')) {
        msg = 'Template incomplete. Please open and Save the template in Editor first.';
      }
      showToast(msg, 'error');
    } finally {
      setDlId(null);
  ...
  const handleCertReqSubmit = async () => {
    ...
    try {
      await submitCertificateRequest({
        ...
        certificateType:
          templates.find((t) => t._id === certReqForm.templateId)?.name || certReqForm.templateId,
  ...
  if (!student) {
    return (
      <div className="p-8 text-center">
        ...
        <button
          onClick={() => navigate('/admin/admissions/students')}
    ...
    <>
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex gap-2 flex-shrink-0">
              {editMode ? (
                ...
                <>
                  <button
                    onClick={() => setShowCertReqModal(true)}
            ...
            {activeTab === 'Summary' && (
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-slate-700">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${form.feesCleared ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}
                        >
                          <CheckCircle
                            size={15}
                            className={form.feesCleared ? 'text-green-600' : 'text-red-500'}
                          />
                        </div>
                        <div>
                          ...
                          <p
                            className={`text-sm font-medium mt-0.5 ${form.feesCleared ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                          >
                      ...
                      {editMode && (
                        ...
                        >
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${form.feesCleared ? 'translate-x-6' : 'translate-x-1'}`}
                          />
                        </button>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-slate-700">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${form.isDebarred ? 'bg-red-50 dark:bg-red-900/20' : 'bg-green-50 dark:bg-green-900/20'}`}
                        >
                          <AlertCircle
                            size={15}
                            className={form.isDebarred ? 'text-red-500' : 'text-green-600'}
                          />
                        </div>
                        <div>
                          <p className="text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide">
                      ...
                      {editMode && (
                        ...
                        >
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${form.isDebarred ? 'translate-x-6' : 'translate-x-1'}`}
                          />
                        </button>
                      )}
                    </div>
                ...
                <div>
                  ...
                  <Field
                    icon={Mail}
                    label="College Email (auto-generated)"
                    name="email"
                    value={form.email}
                    editMode={false}
                    onChange={handleChange}
                ...
                {certsLoading ? (
                  <div className="space-y-3">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-14 bg-gray-100 dark:bg-slate-700 rounded-xl animate-pulse"
                      />
                    ))}
                  </div>
                      ...
                      <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
                        {studentCerts.map((c) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">
                              {c.generatedDate
                                ? new Date(c.generatedDate).toLocaleDateString('en-IN', {
                                    day: 'numeric',
                                    month: 'short',
                                    year: 'numeric',
                                  })
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  title={
                                    c.status === 'Rejected'
                                      ? 'Certificate rejected — cannot download'
                                      : c.status === 'Pending'
                                        ? 'Awaiting approval before download'
                                        : 'Download PDF'
                                  }
                          ...
                          >
                            ...
                            {selTemplate && !tmpl?.fields?.length && (
                              <p className="text-xs text-gray-400 dark:text-slate-500 bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3">
                                This template has no dynamic fields. The certificate will use the
                                pre-written notes as-is.
                              </p>
                            )}
                          </div>
      ...
      {showCertReqModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowCertReqModal(false);
          }}
        >
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-700">
              ...
              <button
                onClick={() => setShowCertReqModal(false)}
                className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700"
              >
                <X size={18} />
              </button>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                {certReqErrors.templateId && (
                  <p className="text-xs text-red-500 mt-1">{certReqErrors.templateId}</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {certReqErrors.reason && (
                  <p className="text-xs text-red-500 mt-1">{certReqErrors.reason}</p>
                )}
              </div>
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100 dark:border-slate-700">
              ...
              <button
                onClick={handleCertReqSubmit}
                disabled={certReqSubmitting}

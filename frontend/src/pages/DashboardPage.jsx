...
import { Users, ClipboardList, GraduationCap, TrendingUp, AlertCircle } from 'lucide-react';
    ...
    <div className="p-6 space-y-6">
      ...
      {detained > 0 && (
        <div className="flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-5 py-4">
          ...
          <button
            type="button"
            onClick={() => { console.log('[Dashboard] Review detained clicked'); navigate('/admin/admissions/students'); }}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-700">
          ...
          <Link to="/admin/admissions/students" className="text-xs text-blue-600 font-medium hover:underline">
            View all →
          </Link>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 dark:bg-slate-900/50">
              {['USN', 'Name', 'Program', 'Semester', 'Status'].map((h) => (
                <th key={h} className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-slate-700">
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="px-6 py-10 text-center text-sm text-gray-400 dark:text-slate-500">
                  No students yet. <Link to="/admin/admissions/students" className="text-blue-600 hover:underline">Add the first one →</Link>
                </td>
              </tr>
            ) : (
              recent.map((s) => (
                ...
                >
                  <td className="px-6 py-3 font-mono text-xs text-gray-500 dark:text-slate-400">{s.student_id}</td>
                  <td className="px-6 py-3 font-medium text-gray-900 dark:text-white">{s.name}</td>
                  <td className="px-6 py-3 text-gray-500 dark:text-slate-400">{s.program || '—'}</td>

...
const ADMISSION_TYPE_META = {
  ...
  lateral: {
    label: 'Lateral Admission',
    term: 3,
    year: 2,
    desc: '2nd Year Direct Entry',
    color: 'purple',
  },
};
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <p
            className={`text-sm font-semibold ${config.enabled ? `text-${c}-700 dark:text-${c}-400` : 'text-gray-700 dark:text-slate-300'}`}
          >
            {meta.label}
          </p>
          <p
            className={`text-xs mt-0.5 ${config.enabled ? `text-${c}-500 dark:text-${c}-400` : 'text-gray-400 dark:text-slate-500'}`}
          >
        ...
        {config.enabled && (
          <span
            className={`text-[10px] font-bold px-2.5 py-1 rounded-full bg-${c}-100 dark:bg-${c}-900/40 text-${c}-700 dark:text-${c}-400 shrink-0`}
          >
            T{meta.term}
          </span>
        )}
...
export default function EnquiryPage() {
  ...
  const [schedules, setSchedules] = useState(() => qc.get('schedules')?.data || []);
  const [loading, setLoading] = useState(() => !qc.has('schedules'));
  ...
  const fetchSchedules = useCallback(async () => {
    setLoadError(false);
    // Skip loading spinner if we already have cached data
    if (!qc.has('schedules')) setLoading(true);
    try {
  ...
  const filtered = schedules.filter((s) => {
    ...
    return (
      s.scheduleName.toLowerCase().includes(q) || (s.academicYear || '').toLowerCase().includes(q)
    );
  });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-80 shadow-2xl">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-5">
              All enquiries linked to this schedule will lose their schedule reference.
            </p>
            ...
            <form onSubmit={handleCreate} className="px-6 py-5 space-y-4 overflow-y-auto">
              <div>
                ...
                <input
                  ...
                  onChange={set('scheduleName')}
                  placeholder="e.g. UG Engineering 2025"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={set('regPrefix')}
                    placeholder="REG25"
                    className={inp(errors.regPrefix)}
                  />
                  {errors.regPrefix && (
                    <p className="text-xs text-red-500 mt-1">{errors.regPrefix}</p>
                  )}
                </div>
                <div>
                  {lbl('Applicant Prefix', true)}
                  <input
                    value={form.applicantPrefix}
                    onChange={set('applicantPrefix')}
                    placeholder="APP25"
                    className={inp(errors.applicantPrefix)}
                  />
                  {errors.applicantPrefix && (
                    <p className="text-xs text-red-500 mt-1">{errors.applicantPrefix}</p>
                  )}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <AdmissionTypeCard
                    category="regular"
                    config={form.admissionType.regular}
                    onToggle={() => toggleAdmType('regular')}
                  />
                  <AdmissionTypeCard
                    category="lateral"
                    config={form.admissionType.lateral}
                    onToggle={() => toggleAdmType('lateral')}
                  />
                </div>
              </div>
              <div className="flex gap-3 pt-1">
                <button
                  type="button"
                  onClick={() => setShowCreate(false)}
                  className="flex-1 py-2.5 text-sm rounded-xl border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300"
                >
                  Cancel
                </button>
                <button
            ...
            <form onSubmit={handleUpdate} className="px-6 py-5 space-y-4 overflow-y-auto">
              <div>
                ...
                <input
                  ...
                  onChange={setEdit('scheduleName')}
                  placeholder="e.g. UG Engineering 2025"
                  className={inp(editErrors.scheduleName)}
                />
                {editErrors.scheduleName && (
                  <p className="text-xs text-red-500 mt-1">{editErrors.scheduleName}</p>
                )}
              </div>
              <div>
                {lbl('Academic Year', true)}
                <select
                  ...
                  onChange={setEdit('academicYear')}
                  className={inp(editErrors.academicYear)}
                >
                  <option value="">— Select —</option>
                  {YEARS.map((y) => (
                    <option key={y} value={y}>
                ...
                {editErrors.academicYear && (
                  <p className="text-xs text-red-500 mt-1">{editErrors.academicYear}</p>
                )}
              </div>
              <div>
                {lbl('Max Seats', true)}
                <input
                  ...
                  onChange={setEdit('maxSeats')}
                  className={inp(editErrors.maxSeats)}
                />
                {editErrors.maxSeats && (
                  <p className="text-xs text-red-500 mt-1">{editErrors.maxSeats}</p>
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  {lbl('Reg Prefix', true)}
                  <input
                    value={editForm.regPrefix}
                    onChange={setEdit('regPrefix')}
                    placeholder="REG25"
                    className={inp(editErrors.regPrefix)}
                  />
                  {editErrors.regPrefix && (
                    <p className="text-xs text-red-500 mt-1">{editErrors.regPrefix}</p>
                  )}
                </div>
                <div>
                  {lbl('Applicant Prefix', true)}
                  <input
                    value={editForm.applicantPrefix}
                    onChange={setEdit('applicantPrefix')}
                    placeholder="APP25"
                    className={inp(editErrors.applicantPrefix)}
                  />
                  {editErrors.applicantPrefix && (
                    <p className="text-xs text-red-500 mt-1">{editErrors.applicantPrefix}</p>
                  )}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <AdmissionTypeCard
                    category="regular"
                    config={editForm.admissionType.regular}
                    onToggle={() => toggleEditAdmType('regular')}
                  />
                  <AdmissionTypeCard
                    category="lateral"
                    config={editForm.admissionType.lateral}
                    onToggle={() => toggleEditAdmType('lateral')}
                  />
                </div>
              </div>
              <div className="flex gap-3 pt-1">
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
              Select a schedule to manage its student pipeline
            </p>
          </div>
          <button
            onClick={() => {
        ...
        {loadError && (
          <div className="flex flex-col items-center justify-center py-16 text-center bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-100 dark:border-red-800">
            ...
            <p className="text-xs text-red-500 dark:text-red-500 mt-1 mb-4">
        ...
        {!loadError && loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-5 space-y-3"
              >
                {[60, 40, 80, 40].map((w, j) => (
                  <div
                    key={j}
                ...
                >
                  ...
                  <div className="p-5 flex-1 space-y-4">
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span
                          className={`font-semibold ${seatsFull ? 'text-red-600 dark:text-red-400' : 'text-gray-700 dark:text-slate-300'}`}
                        >
                      ...
                      <div className="w-full h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${seatsFull ? 'bg-red-500' : 'bg-green-500'}`}
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-slate-400">
                      <Users size={12} />
                      <span>
                        {s.applicantCount || 0} applicant{s.applicantCount !== 1 ? 's' : ''}{' '}
                        registered
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="px-5 py-4 border-t border-gray-50 dark:border-slate-700 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => navigate(`/admin/admissions/enquiry/${s._id}`)}

...
function getStage(enq) {
  ...
  if (enq.admissionStage === 'Application' || enq.admissionStage === 'Verified')
    return 'Application';
  return 'Inquiry';
}
...
function getTermForCategory(category, schedule) {
  ...
  if (detail?.terms?.[0]) {
    ...
    return {
      ...
      label: `Term ${t.term} · Year ${t.year_of_study}`,
    };
  }
...
function getStatusBadge(enq) {
  ...
  if (stage === 'Cancelled')
    return {
      label: 'Cancelled',
      cls: 'bg-gray-200 dark:bg-slate-600 text-gray-500 dark:text-slate-400',
    };
  return {
    label: 'Pending',
...
export default function ScheduleDetailPage() {
  ...
  const stageCounts = useMemo(() => {
    ...
    enquiries.forEach((e) => {
      counts[getStage(e)] = (counts[getStage(e)] || 0) + 1;
    });
    return counts;
  }, [enquiries]);
  ...
  const analytics = useMemo(() => {
    ...
    const seatPct =
      schedule?.maxSeats > 0
        ? Math.round(((schedule.filledSeats || 0) / schedule.maxSeats) * 100)
        : 0;
  ...
  const timelineGroups = useMemo(() => {
    ...
    enquiries.forEach((enq) => {
      events.push({
        ...
        label: `${enq.name} added (Inquiry)`,
        type: 'add',
      });
      if (enq.admissionStage === 'Application')
        events.push({
          date: new Date(enq.updatedAt),
          label: `${enq.name} → Application`,
          type: 'stage',
        });
      if (enq.admissionStage === 'Admitted')
        events.push({
          date: new Date(enq.updatedAt),
          label: `${enq.name} admitted`,
          type: 'admit',
        });
      (enq.followUps || []).forEach((fu) => {
        events.push({
          date: new Date(fu.date),
          label: `${enq.name} — ${fu.note || fu.status || 'Follow-up'}`,
          type: 'followup',
        });
      });
    });
    ...
    events.slice(0, 60).forEach((ev) => {
      const key = ev.date.toLocaleDateString('en-IN', {
        day: 'numeric',
        month: 'long',
        year: 'numeric',
      });
  ...
  const handleAddStudent = async (ev) => {
    ...
    try {
      ...
      const payload = {
        ...
        fatherContact: addForm.fatherContact
          ? `+91${addForm.fatherContact.replace(/\D/g, '').slice(-10)}`
          : '',
        ...
        motherContact: addForm.motherContact
          ? `+91${addForm.motherContact.replace(/\D/g, '').slice(-10)}`
          : undefined,
        ...
        documents:
          addForm.admissionCategory && STD_DOCS[addForm.admissionCategory]
      ...
      let admitted = false;
      try {
        await convertEnquiry(newEnquiry._id, { force: true, degree: addForm.degree || undefined });
        admitted = true;
      } catch (convertErr) {
        // Conversion failed (e.g. seats full) — student is still saved as an enquiry
        toast(
          `Saved as enquiry. Auto-admit failed: ${convertErr.response?.data?.error || 'seats may be full'}`,
          'error'
        );
      }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setEnquiries((p) =>
        p.map((e) =>
          e._id === convertTarget._id
            ? { ...e, status: 'Converted', admissionStage: 'Admitted' }
            : e
        )
      );
      getSchedule(schId)
        .then(({ data: sd }) => setSchedule(sd.data))
        .catch(() => {});
  ...
  const roInp =
    'w-full px-4 py-3 text-sm rounded-xl border border-gray-200 dark:border-slate-600 bg-gray-100 dark:bg-slate-700/50 text-gray-500 dark:text-slate-400 cursor-default focus:outline-none';
          ...
          <div className="max-w-4xl space-y-6">
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    {addErrors.gender && (
                      <p className="text-xs text-red-500 mt-1">{addErrors.gender}</p>
                    )}
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex">
                      <span className="px-3 py-3 text-sm bg-gray-100 dark:bg-slate-600 text-gray-600 dark:text-slate-300 border border-r-0 border-gray-200 dark:border-slate-600 rounded-l-xl select-none font-mono">
                        +91
                      </span>
                      <input
                        type="tel"
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    {lbl('Sub Caste')}
                    <input
                      value={addForm.subCaste}
                      onChange={fa('subCaste')}
            ...
            <section className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
              ...
              <div className="px-6 py-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={fa('degree')}
                      className={inp(addErrors.degree)}
                    >
                    ...
                    {addErrors.degree && (
                      <p className="text-xs text-red-500 mt-1">{addErrors.degree}</p>
                    )}
                    <p className="text-xs text-gray-400 mt-1">
                      Auto-filled from schedule · edit if needed
                    </p>
                  </div>
                  <div>
                    {lbl('Academic Year')}
                ...
                <div>
                  ...
                  {addErrors.program && (
                    <p className="text-xs text-red-500 mt-1">{addErrors.program}</p>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    {lbl('Admission Type')}
                    <select
                      value={addForm.admissionCategory}
                      onChange={(e) =>
                        setAddForm((p) => ({ ...p, admissionCategory: e.target.value }))
                      }
                      className={inp(false)}
                    >
              ...
              <div className="px-6 py-5 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={fa('admissionDate')}
                    className={inp(addErrors.admissionDate)}
                  />
                  {addErrors.admissionDate && (
                    <p className="text-xs text-red-500 mt-1">{addErrors.admissionDate}</p>
                  )}
                </div>
                <div>
                  {lbl('Admission Mode')}
            ...
            <section className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
              ...
              <div className="px-6 py-5 grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={addForm.examRank}
                    onChange={fa('examRank')}
                ...
                <div>
                  ...
                  <input
                    value={addForm.hallTicketNo}
                    onChange={fa('hallTicketNo')}
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={addForm.admOrderDate}
                    onChange={fa('admOrderDate')}
                    className={inp(false)}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="date"
                    value={addForm.issuedDate}
                    onChange={fa('issuedDate')}
                    className={inp(false)}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="date"
                    value={addForm.allotmentDate}
                    onChange={fa('allotmentDate')}
                    className={inp(false)}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="date"
                    value={addForm.lastJoiningDate}
                    onChange={fa('lastJoiningDate')}
                    className={inp(false)}
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    value={addForm.admOrderNumber}
                    onChange={fa('admOrderNumber')}
                ...
                <div>
                  ...
                  <input
                    value={addForm.prevQualification}
                    onChange={fa('prevQualification')}
                    placeholder="e.g. Diploma, PUC"
                    className={inp(false)}
                  />
                </div>
              </div>
            ...
            <section className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
              ...
              <div className="px-6 py-5 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    value={addForm.prevBoard}
                    onChange={fa('prevBoard')}
                    placeholder="e.g. Karnataka Board"
                    className={inp(false)}
                  />
                </div>
                <div>
                  ...
                  <input
                    value={addForm.prevCollege}
                    onChange={fa('prevCollege')}
                ...
                <div>
                  ...
                  <input
                    type="number"
                    step="0.01"
                    value={addForm.prevPercentage}
                    onChange={fa('prevPercentage')}
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={addForm.prevYearPassing}
                    onChange={fa('prevYearPassing')}
            ...
            <section className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
              ...
              <div className="px-6 py-5 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={fa('fatherName')}
                        placeholder="Father's full name"
                        className={inp(addErrors.fatherName)}
                      />
                      {addErrors.fatherName && (
                        <p className="text-xs text-red-500 mt-1">{addErrors.fatherName}</p>
                      )}
                    </div>
                    <div>
                      ...
                      <div className="flex">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setAddErrors((p) => ({ ...p, fatherContact: '' }));
                          }}
                          placeholder="9876543210"
                      ...
                      {addErrors.fatherContact && (
                        <p className="text-xs text-red-500 mt-1">{addErrors.fatherContact}</p>
                      )}
                    </div>
                    <div>
                      {lbl('Occupation')}
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        value={addForm.fatherIncome}
                        onChange={fa('fatherIncome')}
                    ...
                    <div>
                      {lbl('PAN')}
                      <input
                        value={addForm.fatherPan}
                        onChange={fa('fatherPan')}
                        placeholder="e.g. ABCDE1234F"
                        maxLength={10}
                        className={inp(false)}
                      />
                ...
                {/* Mother */}
                <div>
                  <p className="text-xs font-semibold text-pink-600 dark:text-pink-400 uppercase tracking-widest mb-3">
                    Mother
                  </p>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={fa('motherName')}
                        placeholder="Mother's full name"
                        className={inp(addErrors.motherName)}
                      />
                      {addErrors.motherName && (
                        <p className="text-xs text-red-500 mt-1">{addErrors.motherName}</p>
                      )}
                    </div>
                    <div>
                      {lbl('Mother Contact')}
                      <div className="flex">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            setAddForm((p) => ({
                              ...p,
                              motherContact: e.target.value.replace(/\D/g, '').slice(0, 10),
                            }));
                          }}
                          placeholder="9876543210"
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        value={addForm.motherIncome}
                        onChange={fa('motherIncome')}
                    ...
                    <div>
                      {lbl('PAN')}
                      <input
                        value={addForm.motherPan}
                        onChange={fa('motherPan')}
                        placeholder="e.g. ABCDE1234F"
                        maxLength={10}
                        className={inp(false)}
                      />
              ...
              <div className="px-6 py-5 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={fa('addressLine')}
                    placeholder="Door no., Street, Area"
                    className={inp(addErrors.addressLine)}
                  />
                  {addErrors.addressLine && (
                    <p className="text-xs text-red-500 mt-1">{addErrors.addressLine}</p>
                  )}
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={fa('city')}
                      placeholder="City"
                      className={inp(addErrors.city)}
                    />
                    {addErrors.city && (
                      <p className="text-xs text-red-500 mt-1">{addErrors.city}</p>
                    )}
                  </div>
                  <div>
                    {lbl('State', true)}
                    <input
                      value={addForm.stateVal}
                      onChange={fa('stateVal')}
                      placeholder="State"
                      className={inp(addErrors.stateVal)}
                    />
                    {addErrors.stateVal && (
                      <p className="text-xs text-red-500 mt-1">{addErrors.stateVal}</p>
                    )}
                  </div>
                  <div>
                    {lbl('Pincode', true)}
                    <input
                      ...
                      maxLength={6}
                      placeholder="560001"
                      className={inp(addErrors.pincode)}
                    />
                    {addErrors.pincode && (
                      <p className="text-xs text-red-500 mt-1">{addErrors.pincode}</p>
                    )}
                  </div>
                </div>
              </div>
  ...
  return (
    <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="date"
                    value={fuForm.date}
                    onChange={(e) => setFuForm((p) => ({ ...p, date: e.target.value }))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm">
              ...
              {docsPatch.map((doc, i) => (
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      setDocsPatch((p) =>
                        p.map((d, j) => (j === i ? { ...d, submitted: e.target.checked } : d))
                      )
                    }
                    className="w-4 h-4 rounded accent-green-600"
                  />
                  <span
                    className={`text-sm ${doc.submitted ? 'text-green-700 dark:text-green-400' : 'text-gray-700 dark:text-slate-300'}`}
                  >
                    {doc.name}
                  </span>
                  {doc.submitted && (
                    <CheckCircle size={13} className="ml-auto text-green-500 shrink-0" />
                  )}
                </label>
              ))}
            </div>
            <div className="px-6 pb-5 flex gap-3">
              <button
                onClick={() => setDocsTarget(null)}
                className="flex-1 py-2 rounded-xl border border-gray-300 dark:border-slate-600 text-sm text-gray-700 dark:text-slate-300"
              >
                Cancel
              </button>
              <button
      ...
      {convertTarget &&
        (() => {
          const missing = (convertTarget.documents || [])
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-96 shadow-2xl space-y-4">
                ...
                <p className="text-sm text-gray-500">
                  A student record will be created for <strong>{convertTarget.name}</strong>.
                </p>
                <div
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                Here you can manage all your students under schedule
              </p>
            </div>
          </div>
          <button
            onClick={() => {
              setAddForm({ ...FULL_FORM_INIT, degree: schedule?.degree || '' });
              setAddErrors({});
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
            ...
            <div className="border-b border-gray-100 dark:border-slate-700 overflow-x-auto">
              <div className="flex min-w-max">
                ...
                >
                  All
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded font-semibold ${pipelineStage === 'all' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400'}`}
                  >
                    {stageCounts.all}
                  </span>
                </button>
                {PIPELINE_STAGES.map((stage) => (
                  ...
                  >
                    {stage}
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded font-semibold ${pipelineStage === stage ? STAGE_STYLE[stage] : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400'}`}
                    >
                      {stageCounts[stage] || 0}
                    </span>
                  </button>
            ...
            <div className="px-5 py-3 border-b border-gray-50 dark:border-slate-700/50 flex items-center gap-3">
              ...
              <span className="text-xs text-gray-400 dark:text-slate-500">
                from {displayed.length} Students
              </span>
            </div>
                  ...
                  {displayed.length === 0 ? (
                      ...
                      <td colSpan={8} className="px-6 py-14 text-center">
                        ...
                        <p className="text-sm text-gray-400 dark:text-slate-500">
                          {pipelineStage === 'all'
                            ...
                            : `No students in ${pipelineStage} stage`}
                        </p>
                      </td>
                    </tr>
                  ) : (
                    displayed.map((e) => {
                      ...
                      return (
                        <Fragment key={e._id}>
                          <tr
                            className={`hover:bg-gray-50/60 dark:hover:bg-slate-700/30 transition-colors border-l-2 ${STAGE_BORDER[stage]}`}
                          >
                            {/* USN */}
                            ...
                            <td className="px-4 py-3 whitespace-nowrap">
                              <span
                                className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-bold ${status.cls}`}
                              >
                            ...
                            {/* Email */}
                            <td className="px-4 py-3 text-xs text-gray-500 dark:text-slate-400 max-w-[150px] truncate">
                              {e.email}
                            </td>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {PIPELINE_STAGES.map((stage) => (
                <div
                  key={stage}
                  className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-4 text-center"
                >
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                    {stageCounts[stage] || 0}
                  </p>
                  <span
                    className={`inline-flex mt-1 px-2 py-0.5 rounded-full text-xs font-semibold ${STAGE_STYLE[stage]}`}
                  >
                    {stage}
                  </span>
                </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
                ...
                <div className="flex items-end gap-4">
                  ...
                  <p className="text-sm text-gray-500 pb-1">
                    {schedule.filledSeats}/{schedule.maxSeats} seats
                  </p>
                </div>
                <div className="mt-3 h-3 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${analytics.seatPct >= 90 ? 'bg-red-500' : analytics.seatPct >= 70 ? 'bg-amber-500' : 'bg-green-500'}`}
                  ...
                  <div className="space-y-2 pl-3 border-l-2 border-gray-100 dark:border-slate-700">
                    {events.map((ev, i) => (
                      <div key={i} className="flex gap-3 items-start">
                        <div
                          className={`w-2 h-2 rounded-full mt-1.5 shrink-0 -ml-[5px] ${ev.type === 'add' ? 'bg-blue-400' : ev.type === 'admit' ? 'bg-green-400' : ev.type === 'followup' ? 'bg-amber-400' : 'bg-gray-300'}`}
                        />

...
function StatusBadge({ status }) {
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_CLS[status] || 'bg-gray-100 text-gray-600'}`}
    >
...
export default function CertificatesPage() {
  ...
  const [activeTab, setActiveTab] = useState(
    () => localStorage.getItem('certificates_active_tab') || 'Issued'
  );
  ...
  useEffect(() => {
    ...
    fetchTemplates();
    fetchCertRequests();
  }, [fetchCerts, fetchTemplates, fetchCertRequests]);
  ...
  const handleRegenerate = async (cert) => {
    ...
    try {
      ...
      setCerts((p) => p.map((c) => (c._id === cert._id ? { ...c, ...updated } : c)));
      toast('PDF regenerated successfully!');
    } catch (err) {
      let msg = err?.response?.data?.error || err.message || 'PDF regeneration failed';
  ...
  // ── Derived ───────────────────────────────────────────────────────────────
  const pendingCerts = certRequests.filter((r) => r.status === 'Pending');
          ...
          <div className="flex border-b border-gray-100 dark:border-slate-700 px-1 pt-1">
            {TABS.map((tab) => (
              <button
                ...
                onClick={() => {
                  setActiveTab(tab);
                  localStorage.setItem('certificates_active_tab', tab);
                }}
                  ...
                  {certLoad ? (
                    ...
                    certs.map((c) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          {/* Cert is Approved or Generated → show Download button */}
                          {c.status === 'Generated' || c.status === 'Approved' ? (
                            <button
                              id={`download-cert-${c._id}`}
                              onClick={() => handleDownload(c)}
                              disabled={dlId === c._id}
                              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors bg-blue-50 dark:bg-blue-900/20 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/40 cursor-pointer"
                            >
                              {dlId === c._id ? (
                                <svg
                            ...
                            <button
                              ...
                              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-gray-100 dark:bg-slate-700 text-gray-400 dark:text-slate-500 cursor-not-allowed opacity-60"
                            >
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
                  {tmplLoad ? (
                    ...
                    <EmptyRow
                      cols={6}
                      icon={FileText}
                      text='No templates yet — click "+ Add Template" to create one'
                    />
                  ) : (
                    templates.map((t) => (
                      <tr
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-gray-500 dark:text-slate-400 max-w-[220px] truncate">
                          {t.description || '—'}
                        </td>
                  ...
                  <div className="space-y-5">
                    {timelineEvents.map((ev) => (
                      <div key={ev.id} className="flex gap-4 items-start">
                        <div
                          className={`w-3 h-3 rounded-full mt-1 shrink-0 ring-2 ring-white dark:ring-slate-800
                          ${
                            ev.status === 'Generated'
                              ? 'bg-green-500'
                              : ev.status === 'Approved'

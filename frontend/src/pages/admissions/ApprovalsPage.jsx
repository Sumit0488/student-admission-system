...
import { getApprovals, updateApproval, getAllCertificateRequests, updateCertificateRequest } from '../../services/admissionsApi';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-slate-700">
          ...
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-600 dark:hover:text-slate-300">
            <X size={18} />
          </button>
        </div>
...
function ApprovalsTab({ toast }) {
  ...
  const handleStatus = async (id, status) => {
    setUpdating(id);
    try {
      const { data } = await updateApproval(id, { status });
      setApprovals((p) => p.map((a) => a._id === id ? data.data : a));
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead>
            <tr className="bg-gray-50 dark:bg-slate-900/50">
              {['Student Name', 'USN', 'Certificate', 'Requested', 'Status', 'Actions'].map((h) => (
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
            {loading ? Array.from({ length: 4 }).map((_, i) => (
              <tr key={i}>{Array.from({ length: 6 }).map((__, j) => (
                <td key={j} className="px-4 py-3"><div className="h-3 bg-gray-200 dark:bg-slate-700 rounded animate-pulse w-24" /></td>
              ))}</tr>
            )) : approvals.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-16 text-center">
                <CheckCircle size={32} className="mx-auto mb-2 text-gray-300 dark:text-slate-600" />
                <p className="text-sm text-gray-400 dark:text-slate-500">
                  {statusFilter === 'All' ? 'No approvals yet' : `No ${statusFilter.toLowerCase()} requests`}
                </p>
              </td></tr>
            ) : approvals.map((a) => (
              <tr key={a._id} className="hover:bg-gray-50/60 dark:hover:bg-slate-700/30 transition-colors">
                <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{a.studentName}</td>
                <td className="px-4 py-3 font-mono text-xs text-gray-500 dark:text-slate-400">{a.usn}</td>
                <td className="px-4 py-3 text-gray-600 dark:text-slate-300">{a.certificate}</td>
                <td className="px-4 py-3 text-gray-400 dark:text-slate-500 text-xs whitespace-nowrap">
                  {new Date(a.requestedDate).toLocaleDateString('en-IN')}
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_STYLE[a.status]}`}>
      ...
      {!loading && approvals.length > 0 && (
        <div className="px-5 py-3 border-t border-gray-100 dark:border-slate-700">
          <p className="text-xs text-gray-400 dark:text-slate-500">
            Showing <span className="font-semibold text-gray-600 dark:text-slate-300">{approvals.length}</span> result{approvals.length !== 1 ? 's' : ''}
...
function StudentRequestsTab({ toast }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch { toast('Failed to load student requests', 'error'); }
    finally { setLoading(false); }
  }, [statusFilter, search]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[900px]">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
              {loading ? Array.from({ length: 4 }).map((_, i) => (
                <tr key={i}>{Array.from({ length: COLS.length }).map((__, j) => (
                ...
                <tr key={r._id} className="hover:bg-gray-50/60 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_STYLE[r.status]}`}>
...
export default function ApprovalsPage() {
  ...
  const TABS = [
    ...
    { key: 'studentRequests',  label: 'Student Requests', icon: <FileText size={14} />    },
  ];
    ...
    <>
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Review and action certificate requests</p>
        </div>
        ...
        {activeTab === 'approvals'       && <ApprovalsTab       toast={toast} />}
        {activeTab === 'studentRequests' && <StudentRequestsTab toast={toast} />}
      </div>
    </>
  );
}

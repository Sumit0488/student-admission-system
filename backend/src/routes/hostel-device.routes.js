...
router.post('/', async (req, res) => {
  try {
    ...
    log('device_added', 'Device Added', { entity_id: rec.device_id, entity_label: rec.device_name, student_name: rec.student_name, usn: rec.usn, details: `${rec.device_type} device registered` });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (!rec) return res.status(404).json({ message: 'Not found' });
    log('device_updated', 'Device Updated', { entity_id: rec.device_id, entity_label: rec.device_name, student_name: rec.student_name, details: `Status: ${rec.status}` });
    res.json(rec);
  } catch (e) { res.status(400).json({ message: e.message }); }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (!rec) return res.status(404).json({ message: 'Not found' });
    log('device_deleted', 'Device Deleted', { entity_id: rec.device_id, entity_label: rec.device_name });
...
router.post('/:id/assign', async (req, res) => {
  try {
    ...
    log('device_assigned', 'Device Assigned', { entity_id: rec.device_id, entity_label: rec.device_name, student_name: rec.student_name, usn: rec.usn, details: `Assigned to ${rec.student_name} - Room ${rec.room_number}` });
    res.json(rec);
  } catch (e) { res.status(400).json({ message: e.message }); }
});
...
router.post('/:id/return', async (req, res) => {
  try {
    ...
    log('device_returned', 'Device Returned', { entity_id: rec.device_id, entity_label: rec.device_name, details: `Returned in ${req.body.condition || 'Good'} condition` });
    res.json(rec);
  } catch (e) { res.status(400).json({ message: e.message }); }
});
